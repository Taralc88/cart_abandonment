...
## Data
```{r}
cart_sessions_matched <- fread("cart_sessions_matched.csv")

customer <- fread("customer.csv") |>
  clean_names() |>
  rename(customer_id = customer_number)

google_analytics <- fread("google_analytics.csv") |>
  clean_names()
...
p +
  ...
  annotate(
    "text",
    x = max(modeling_data_sessions$total_add_to_carts_before, na.rm = TRUE) - 280,
...
cluster_vars <- intersect(cluster_vars, names(modeling_data_sessions))

# Make sure clustering vars are numeric and finite (Inf/NaN → NA)
modeling_data_sessions[, (cluster_vars) := lapply(.SD, function(x) {
  x <- as.numeric(x)
  x[!is.finite(x)] <- NA_real_
  x
}), .SDcols = cluster_vars]

# Build matrix, drop rows with any NA in clustering vars
X <- as.matrix(modeling_data_sessions[, ..cluster_vars])
good_rows <- complete.cases(X)
X <- X[good_rows, , drop = FALSE]

# Drop zero-variance columns
var_ok <- apply(X, 2, function(z) sd(z) > 0)
X <- X[, var_ok, drop = FALSE]

# Scale and run k-means
set.seed(123)
km_fit <- kmeans(scale(X), centers = 4, nstart = 50)

# Attach cluster back to main data.table
modeling_data_sessions[, cluster := NA_integer_]
modeling_data_sessions$cluster[good_rows] <- km_fit$cluster

cluster_summary <- modeling_data_sessions[!is.na(cluster),
  .(
    n_sessions   = .N,
    abandon_rate = mean(session_abandoned == 1)
  ),
  by = cluster
][order(-abandon_rate)]
...
behavior_summary <- modeling_data_sessions[!is.na(cluster),
  lapply(.SD, mean, na.rm = TRUE),
  by = cluster,
  .SDcols = cluster_vars
]

behavior_summary <- merge(
  behavior_summary,
  modeling_data_sessions[!is.na(cluster),
    .(abandon_rate = mean(session_abandoned == 1)),
    by = cluster
  ],
  by = "cluster"
...
ggplot(cluster_summary, aes(x = factor(cluster), y = abandon_rate)) +
  geom_col() +
  geom_text(aes(label = scales::percent(abandon_rate, 0.1)),
            vjust = -0.5, size = 4) +
  labs(
    title = "Abandonment Rate by Behavioral Cluster",
    x = "Cluster",
    y = "Abandonment Rate"
  ) +
  theme_minimal()
...
beh_long <- melt(
  behavior_summary,
  id.vars = "cluster",
  measure.vars = cluster_vars,
  variable.name = "metric",
  value.name = "mean_value"
)

ggplot(beh_long, aes(x = factor(cluster), y = mean_value)) +
  geom_col() +
...
# sample (optional) in case you have a ton of rows
set.seed(123)
plot_data <- modeling_data_sessions[!is.na(cluster)]
if (nrow(plot_data) > 20000) {
  plot_data <- plot_data[sample(.N, 20000)]
}

ggplot(plot_data,
       aes(x = n_adds,
           y = session_duration_mins,
           color = factor(cluster))) +
  geom_point(alpha = 0.3) +
  labs(
    title = "Session Behavior by Cluster",
    x = "Number of adds",
...
# plot_vars <- c("n_adds", "session_duration_mins", "cart_event_count",
#                "begin_checkout", "proceed_to_checkout", "remove_from_cart")
# 
# GGally::ggparcoord(
#   data = behavior_summary,
#   columns = match(plot_vars, names(behavior_summary)),
#   groupColumn = match("cluster", names(behavior_summary)),
#   scale = "std"
# ) +
#   labs(
#     title = "Parallel Coordinates Plot of Cluster Behavioral Profiles",
...
centers <- as.data.frame(km_fit$centers)
centers$cluster <- rownames(centers)

centers_long <- melt(centers, id = "cluster")

ggplot(centers_long, aes(x = variable, y = cluster, fill = value)) +
  geom_tile() +
  scale_fill_viridis_c() +
  labs(
    title = "Heatmap of Cluster Centers",
    x = "Feature",
    y = "Cluster"
  ) +
  theme_minimal() +
