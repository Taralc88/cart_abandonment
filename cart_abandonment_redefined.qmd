...
customer <- fread("customer.csv") |> clean_names() |> 
  rename(customer_id = customer_number)

google_analytics <- fread("google_analytics.csv") |> clean_names()

orders <- fread("orders.csv") |> clean_names()
...
```{r}
write.csv(cart_sessions_matched, "cart_sessions_matched.csv", row.names = FALSE)
```